...
class NNetWrapper(NeuralNet):
    def __init__(self, pq):
        self.pq = pq
        board, _ = pq.getInitBoard()
        attacker_inputs = len(pq.get_attack_vector(board))
        defender_inputs = len(pq.get_defend_vector(board))
        self.att_nnet = PQAC(attacker_inputs, pq.size, 64)
        self.def_nnet = PQAC(defender_inputs, 2 * pq.size, 64)
    ...
    def predict(self, canonicalBoard):
        ...
        if player == 1:
            nnet = self.att_nnet
            board = self.pq.get_attack_vector(board)
        elif player == -1:
            nnet = self.def_nnet
            board = self.pq.get_defend_vector(board)
        else:
            print('burh')
        ...
        if player == 1:
            probs = np.concatenate((probs, np.zeros(2 * self.pq.size)))
        else:
            probs = np.concatenate((np.zeros(self.pq.size), probs))
